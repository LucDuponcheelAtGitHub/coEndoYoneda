...
abbrev G {C : Type (u + 1)} [LargeCategory.{u} C] (F : Type u ⥤ C) (X : C) : Type u :=
  F.obj PUnit ⟶ X
...
theorem GF_map_eq_φ {X Y : C} (f : X ⟶ Y) :
    (GF_def FF).map f = φ (C := C) (X := G FF X) (Y := G FF Y)
    (fun h => h ≫ f) :=
  CYEF_map_eq_φ f
...
def globalFunctorialValueToNaturalTransformation1
  (F : C ⥤ C) {X : C} (g : G FF (F.obj X)) :
    CYEF X ⟶ F ⋙ GF where
  app Y :=
    φ ((fun f => g ≫ F.map f) : (X ⟶ Y) → (G FF (F.obj Y)))
  naturality t t' h := by
...
def globalFunctorialValueToNaturalTransformation2
  (F : C ⥤ C) {X : C} (g : G FF (GF.obj (F.obj X))) :
    CYEF X ⟶ F ⋙ GF where
  app Y :=
    φ ((fun f => g ≫ (F ⋙ GF).map f) :
      (X ⟶ Y) → (G FF (GF.obj (F.obj Y)))) ≫
        γμ.app (F.obj Y)
  naturality t t' h := by
...
theorem coEndoYonedaLemma2 {F : C ⥤ C} {Z : C} (τ : CYEF Z ⟶ F ⋙ GF) :
  ...
  have h_inner_eq : φ (C := C) (X := Z ⟶ Y) (Y := G FF ((F ⋙ GF).obj Y))
    ...
    exact (Eq.trans step1 (Eq.trans step2 (Eq.trans step3 step4))).symm
  have h_final : τ.app Y = φ (C := C) (X := Z ⟶ Y) (Y := G FF ((F ⋙ GF).obj Y))
    (fun f => (φ (C := C) (X := PUnit) (Y := Z ⟶ Z)
...
@[simp]
theorem φ_const_comp_γμ {W : C} (g : G FF ((GF_def FF).obj W)) :
    φ (C := C) (X := PUnit) (Y := G FF ((GF_def FF).obj W))
  ...
  have h_γη : γη.app (FF.obj PUnit) =
    φ (C := C) (X := PUnit) (Y := G FF (FF.obj PUnit))
      (fun w => φ (C := C) (X := PUnit) (Y := PUnit) (fun _ => w)) := γη_φ PUnit
  erw [h_γη] at h_nat
  have h_comp2 : φ (C := C) (X := PUnit) (Y := G FF (FF.obj PUnit))
    (fun w => φ (C := C) (X := PUnit) (Y := PUnit) (fun _ => w)) ≫
      φ (C := C) (X := G FF (FF.obj PUnit)) (Y := G FF ((GF_def FF).obj W))
        (fun k => k ≫ g) =
    φ (C := C) (X := PUnit) (Y := G FF ((GF_def FF).obj W))
  ...
  have h_g_eta : g ≫ γη.app ((GF_def FF).obj W) =
    φ (C := C) (X := PUnit) (Y := G FF ((GF_def FF).obj W)) (fun _ => g) := by
    erw [h_id_w] at h_nat
    exact h_nat
...
def coEndoYonedaEquiv {F : C ⥤ C} {X : C} : (CYEF X ⟶ F ⋙ GF) ≃ (G FF ((F ⋙ GF).obj X))
where
  ...
  right_inv := fun g ↦ by
    change φ (C := C) (X := PUnit) (Y := X ⟶ X) (fun _ => 𝟙 X) ≫
      (φ (C := C) (X := X ⟶ X) (Y := G FF (GF.obj (F.obj X))) (fun f => g ≫ (F ⋙ GF).map f) ≫
        γμ.app (F.obj X)) = g
    rw [← Category.assoc]
    have h1 : φ (C := C) (X := PUnit) (Y := X ⟶ X) (fun _ => 𝟙 X) ≫
      φ (C := C) (X := X ⟶ X) (Y := G FF (GF.obj (F.obj X))) (fun f => g ≫ (F ⋙ GF).map f) =
      φ (C := C) (X := PUnit) (Y := G FF (GF.obj (F.obj X))) (fun _ => g ≫ (F ⋙ GF).map (𝟙 X)) := φ_comp _ _
    rw [h1]
    have h2 : φ (C := C) (X := PUnit) (Y := G FF (GF.obj (F.obj X))) (fun _ => g ≫ (F ⋙ GF).map (𝟙 X)) =
      φ (C := C) (X := PUnit) (Y := G FF (GF.obj (F.obj X))) (fun _ => g) := by
      congr 1
      ext _
